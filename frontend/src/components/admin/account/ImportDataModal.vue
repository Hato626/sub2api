...
const buildDataPayloadFromText = (text: string): AdminDataPayload | null => {
  let parsed: unknown
  try {
...
const buildDataPayloadFromFiles = async (): Promise<AdminDataPayload | null> => {
  ...
  return mergeDataPayloads(dataPayloads)
}
...
const handleImport = async () => {
  ...
  try {
    ...
    if (importSource.value === 'text') {
      ...
      dataPayload = buildDataPayloadFromText(text)
    } else {
      dataPayload = await buildDataPayloadFromFiles()
    }
