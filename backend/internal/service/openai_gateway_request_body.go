...
func extractOpenAIReasoningEffortFromBody(body []byte, modelCandidates ...string) *string {
	...
	if value == "" {
		value = defaultOpenAIReasoningEffortForUsage(firstNonEmpty(modelCandidates...))
	}
	if value == "" {
		return nil
	}
...
func normalizeOpenAIReasoningEffortForModel(raw, model string) string {
	value := strings.ToLower(strings.TrimSpace(raw))
	value = strings.NewReplacer("-", "", "_", "", " ", "").Replace(value)
	if value == "none" && openai.IsGPT6SolOrLunaModelSpelling(model) {
		return "none"
	}
	if value == "max" && supportsOpenAIReasoningEffortMax(model) {
		return "max"
	}
	if value == "ultra" && isOpenAIGPT56Model(model) {
		return "ultra"
	}
